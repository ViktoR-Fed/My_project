# Проект - Улучшение личного кабинета клиента банка

## В этом проекте я делаю улучшения для личного кабинета клиента. Это виджет, который показывает несколько последних успешных банковских операций клиента. На бэкенде будем готовить данные для отображения в новом виджете.

## Функционал проекта.

### Модуль masks
Содержит функции для наложения масок на конфиденциальные данные:

**get_mask_card_number(card_number: Union[int, str]) -> str**

Маскирует номер банковской карты в формате: XXXX XX** **** XXXX
Пример: 7000792289606361 → 7000 79** **** 6361

**get_mask_account(account_number: Union[int, str]) -> str**

Маскирует номер счета в формате: **XXXX
**Пример:** 73654108430135874305 → **4305

### Модуль widget
Содержит функции для работы с банковскими данными:

**mask_account_card(account_card: str) -> str**

Автоматически определяет тип данных (карта или счет) и применяет соответствующую маску.
**Пример для карты:** Visa Platinum 7000792289606361 → Visa Platinum 7000 79** **** 6361
**Пример для счета:** Счет 73654108430135874305 → Счет **4305

**get_date(date: str) -> str**
Функция берёт данные даты и времени в формате "2024-03-11T02:26:18.671407"
и возвращает только дату в формате 'ДД.ММ.ГГГГ'"""

### Модуль processing
 Модуль processing предоставляет функции для фильтрации и сортировки банковских операций.

**filter_by_state(data: List[Dict[str, Any]], state: str = "EXECUTED") -> List[Dict[str, Any]]**
Фильтрует список операций по статусу выполнения.

**Параметры:**

#### data: Список словарей с операциями
#### state: Статус для фильтрации. По умолчанию 'EXECUTED'

**Пример использования:**
```
from processing import filter_by_state
data = [
    {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
    {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'}
    ]
# Фильтрация по статусу 'EXECUTED' (по умолчанию)
executed_operations = filter_by_state(data)
# [{'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'}]
# Фильтрация по статусу 'CANCELED'
canceled_operations = filter_by_state(data, 'CANCELED')
# [{'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'}]
```
**sort_by_date(list_dict: List[Dict[str, Any]], ascending: bool = True) -> List[Dict[str, Any]]**
Сортирует список операций по дате.

**Параметры:**

#### showing: Список словарей с операциями
#### is_ascending: Порядок сортировки(True (по умолчанию) - по убыванию, False - по возрастанию(сначала самые старые))

Пример использования:
```
showing = [
    {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
    {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}
]
# Сортировка по убыванию (сначала новые)
sorted_desc = sort_by_date(showing)
# [{'id': 41428829, ...}, {'id': 939719570, ...}]
# Сортировка по возрастанию (сначала старые)
sorted_asc = sort_by_date(showing,False)
# [{'id': 939719570, ...}, {'id': 41428829, ...}]
```

### Модуль generators
Модуль generators содержит функции для работы с массивами транзакций.

**filter_by_currency(transaction_list: List[Dict[str, Any]], currency_type: str) -> Any**

Функция возвращает итератор, который поочередно выдает транзакции, где валюта операции соответствует заданной (например, USD)

**Параметры:**

#### transaction_list: список словарей, представляющих транзакции.
#### currency_type: Валюта для фильтрации (например, USD). 

Пример использования:
```
usd_transactions = filter_by_currency(transactions, "USD")
for _ in range(2):
    print(next(usd_transactions))

>>> {
          "id": 939719570,
          "state": "EXECUTED",
          "date": "2018-06-30T02:08:58.425572",
          "operationAmount": {
              "amount": "9824.07",
              "currency": {
                  "name": "USD",
                  "code": "USD"
              }
          },
          "description": "Перевод организации",
          "from": "Счет 75106830613657916952",
          "to": "Счет 11776614605963066702"
      }
      {
              "id": 142264268,
              "state": "EXECUTED",
              "date": "2019-04-04T23:20:05.206878",
              "operationAmount": {
                  "amount": "79114.93",
                  "currency": {
                      "name": "USD",
                      "code": "USD"
                  }
              },
              "description": "Перевод со счета на счет",
              "from": "Счет 19708645243227258542",
              "to": "Счет 75651667383060284188"
       }
```

**transaction_descriptions(transactions: List[Dict[str, Any]] | Any) -> Generator**

Функция - генератор, который принимает список словарей с транзакциями и возвращает описание каждой операции по очереди

**Параметры:**

#### transactions: список словарей, представляющих транзакции.

Пример использования:
```
descriptions = transaction_descriptions(transactions)
for _ in range(5):
    print(next(descriptions))

>>> Перевод организации
    Перевод со счета на счет
    Перевод со счета на счет
    Перевод с карты на карту
    Перевод организации
```

**card_number_generator(start: int, stop: int) -> Generator**

Функция - генератор, который выдает номера банковских карт в формате XXXX XXXX XXXX XXXX, где X — цифра номера карты

**Параметры:**

#### start: начальное значения для генерации диапазона номеров.
#### stop: конечное значения для генерации диапазона номеров.

Пример использования:
```
for card_number in card_number_generator(1, 5):
    print(card_number)

>>> 0000 0000 0000 0001
    0000 0000 0000 0002
    0000 0000 0000 0003
    0000 0000 0000 0004
    0000 0000 0000 0005
```

### Модуль decorators
Модуль decorators содержит декораторы для автоматической регистрации деталей выполнения функции.

**log(filename=None):**

Декоратор, который будет автоматически логировать начало и конец выполнения функции,
а также ее результаты или возникшие ошибки.

**Параметры:**
#### filename: необязательный аргумент, который определяет, куда будут записываться логи (в файл или в консоль).

Пример использования:
```
@log()
def add_numbers(a, b):
    return a + b

add_numbers(1,2)

>>> Функция add_numbers начинает выполняться.

    add_numbers успешно выполнена. Результат: 3
```

### Модуль external_api
**currency_conversion(transaction: List[Dict[str, Any]]) -> float**

Функция, которая принимает на вход транзакцию и возвращает сумму транзакции

**Параметры:**
#### transaction: транзакция, обернутая в список

Пример использования:
```
result = currency_conversion({
    "id": 41428829,
    "state": "EXECUTED",
    "date": "2019-07-03T18:35:29.512364",
    "operationAmount": {
      "amount": "8221.37",
      "currency": {
        "name": "USD",
        "code": "USD"
      }
    },
    "description": "Перевод организации",
    "from": "MasterCard 7158300734726758",
    "to": "Счет 35383033474447895560"
  })
print(result)

>>> 661683.210098
```

### Модуль utils
**receiving_financial_transactions(filepath: str = "data/operations.json") -> List[Dict[str, Any]]**

Функция, которая получает данные о финансовых транзакциях из JSON-файла, по указанному пути до файла с транзакциями.

**Параметры:**
#### filepath: путь до файла с транзакциями 

Пример использования:
```
result = receiving_financial_transactions("C:/Users/20vik/myproject/My_project/data/operations.json")
print(result)

>>> [{'id': 441945886, 
'state': 'EXECUTED', 
'date': '2019-08-26T10:50:58.294041', 
'operationAmount': 
    {'amount': '31957.58', 
    'currency': 
        {'name': 'руб.', 
        'code': 'RUB'}
    }, 
'description': 'Перевод организации', 
'from': 'Maestro 1596837868705199', 
'to': 'Счет 64686473678894779589'} ....],
```

### Модуль data_processing
**unpacking_csv(filename: str = "data/transactions.csv") -> List[Dict[str, Any]]:**

Функция, которая получает данные о финансовых транзакциях из csv-файла, по указанному пути до файла с транзакциями

**Параметры:**
#### filename: путь до csv-файла 
```
result = unpacking_csv()
print(result)

>>> [{'id': '650703',
 'state': 'EXECUTED', 
 'date': '2023-09-05T11:30:32Z', 
 'amount': '16210',
 'currency_name': 'Sol',
 'currency_code': 'PEN', 
 'from': 'Счет 58803664561298323391',
 'to': 'Счет 39745660563456619397', 
 'description': 'Перевод организации'} ....],
```

**unpacking_xlsx(filename: str = "data/transactions_excel.xlsx") -> List[Dict[str, Any]]::**

Функция, которая получает данные о финансовых транзакциях из xlsx-файла, по указанному пути до файла с транзакциями

**Параметры:**
#### filename: путь до xlsx-файла 

```
result = unpacking_xlsx()
print(result)

>>> [{'id': 650703.0,
 'state': 'EXECUTED',
 'date': '2023-09-05T11:30:32Z',
 'amount': 16210.0,
 'currency_name': 'Sol', 
 'currency_code': 'PEN',
 'from': 'Счет 58803664561298323391',
 'to': 'Счет 39745660563456619397',
 'description': 'Перевод организации'} ....],
```

## Установка:
**Клонируйте репозиторий:**
```
git clone https://github.com/ViktoR-Fed/My_project.git)
```
## Разработка:

- Проект находится в активной разработке. В ближайших планах:

- Добавление новых модулей для расширения функционала

- Улучшение обработки различных форматов данных

- Добавление тестов

## Тестирование модулей банковского приложения

### Обзор тестирования
Использованный инструмент для тестирования - фреймворк  **Pytest**.

Этот проект содержит комплексные тесты для модулей:
- **widget** - функции форматирования и маскирования
- **processing** - функции обработки данных
- **masks** - функции для маскирования счета и номеров банковской карты
- **decorators** - функции для логирования начала и завершения работы функций 
- **external_api** - функции для обработки транзакций
- **generators** - функции - генераторы для работы с транзакциями и с номером карты  
- **data_processing** - функции для получения данных из csv и xlsx - файлов
- **utils** - функции для получения данных из JSON - файлов
## Покрытие тестами

### Модуль widget

#### Функция `get_mask_card_number`
- ✅ Стандартные номера карт (16 цифр)
- ✅ Короткие/длинные номера (граничные случаи)
- ✅ Пустые строки

#### Функция `get_mask_account`
- ✅ Стандартные номера счетов (20 цифр)
- ✅ Короткие/длинные номера
- ✅ Пустые строки

#### Функция `mask_account_card`
- ✅ Автоматическое распознавание типа (карта/счет)
- ✅ Параметризованные тесты разных платежных систем
- ✅ Некорректные входные данные
- ✅ Пустые строки 

#### Функция `get_date`
- ✅ Граничные случаи (пустые строки)
- ✅ Нестандартные строки с датами

### Модуль processing

#### Функция `filter_by_state`
- ✅ Фильтрация по различным статусам
- ✅ Отсутствие элементов с искомым статусом
- ✅ Пустые списки
- ✅ Неверный тип передаваемых данных

#### Функция `sort_by_date`
- ✅ Сортировка по убыванию/возрастанию
- ✅ Одинаковые даты (стабильность сортировки)
- ✅ Неверный тип передаваемых данных
- ✅ Пустые списки

### Модуль generators

#### Функция `filter_by_currency`
- ✅ Фильтрация по заданной валюте
- ✅ Отсутствие элементов с искомым статусом
- ✅ Пустые списки

#### Функция `transaction_descriptions`
- ✅ Возврат корректного описания для каждой транзакции.
- ✅ Пустые списки
- ✅ Неверный тип передаваемых данных

#### Функция `card_number_generator`
- ✅ Обработка крайних значений диапазона
- ✅ Корректность завершения генерации
- ✅ Корректность форматирования номеров карт

### Модуль decorators
#### Декоратор `log`
- ✅ Имя функции и результат выполнения при успешной операции.
- ✅ Имя функции, тип возникшей ошибки и входные параметры, если выполнение функции привело к ошибке.

### Модуль external_api
#### Функция `currency_conversion`
- ✅ Корректность завершения для транзакции в USD
- ✅ Корректность завершения для транзакции в EUR
- ✅ Корректность завершения для транзакции в RUB
- ✅ Обработка ошибки API

### Модуль utils
#### Функция `receiving_financial_transactions`
- ✅ Возврат пустого списка, когда указан несуществующий файл.
- ✅ Возврат пустого списка, когда путь указан неверно.
- ✅ Возврат пустого списка, когда файл пустой.
- ✅ Возврат пустого списка, когда JSON не список.
- ✅ Возврат пустого списка, когда JSON не валидный.

### Модуль data_processing
#### Функция `unpacking_csv`
- ✅ Возврат пустого списка, когда указан несуществующий файл.
- ✅ Возврат пустого списка, когда путь указан неверно.
- ✅ Возврат пустого списка, когда файл пустой.

#### Функция `unpacking_xlsx`
- ✅ Возврат пустого списка, когда указан несуществующий файл.
- ✅ Возврат пустого списка, когда путь указан неверно.
- ✅ Возврат пустого списка, когда файл пустой.


## Запуск тестов

### Все тесты
В терминале ввести: **pytest** 

# Команда проекта
Viktor Fedotov - Beginner Python developer

*Проект разработан в рамках учебного курса SkyPro*